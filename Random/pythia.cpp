/*This script will give us an idea of how much data we need to create  it does the same analysis
as before but this time with the charm qurak.*/
//
...
// execute it with `./pythia "outputfilename.root"`. 
...
int main(int argc, char** argv)
{
	...
	Double_t nParticles = 0;//Number of particles
	...
	TH1D *hLambdaDPhi = new TH1D("hLambdaDPhi", "Delta #varphi between a Xi-hadron other strange hadrons in the same event", 100, -PI/2, 3*PI/2); 
	...
	TH1D *hPDGCharm = new TH1D("hPDGCharm", "PDG codes of charm hadrons", 12000, -6000, 6000);
	TH1D *hNparticles = new TH1D("hNparticles", "Number of particles at each event;Particles/Event",501,-0.5,501.5);
	...
	for(int iEvent = 0; iEvent < nEvents; iEvent++) {
		...
		//At each event we start with zero particles produced
		nParticles = 0;
		...
		for(int iPart = 0; iPart < nPart; iPart++) {
			...
			if(pT < pTmin || eta > etamax ||  eta < -etamax || Q == 0) continue; // if particle fails kinematic checks skip it
			...
			if(IsCharm(pdg)) hPDGCharm->Fill((Double_t) pdg);
			...
			if(abs(pdg) == 4122){ // 4122 is the code for a Lambda^{+}_{c}
				// we found a Lambda^{+}_{c}! let's loop over all the other particles in the event
				...
				for(int jPart = 0; jPart < nPart; jPart++) {
					...
					if(IsCharm(part2pdg) && part2.isFinal() && part2pT > pTmin && part2eta < etamax && part2.charge() != 0){
						Double_t dPhi = DeltaPhi(phi, part2.phi());
						hLambdaDPhi->Fill(dPhi);
					} // end check if 2nd particle passes checks
				} // end 2nd particle loop
		        } // end if-statement checking for Lambda
