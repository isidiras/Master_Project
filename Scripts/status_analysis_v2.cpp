...
//specifically. This can be done through the argument mechanism
//Choose 0 if the analysis is mechanism indepedent
//Choose 1 if the analysis is focused on string fragmentation produced particles
//Choose 2 if the analysis is focused on decay production


TGraphErrors *Yield_Low(TString filename,Double_t start[3], Double_t finish[3],Double_t low_limit, Double_t high_limit, int mechanism){
	...
	const char* DPhi_NamesPr[3]= {"hDPhiLLPr","hDPhiILPr","hDPhiHLPr"};
	const char* DPhi_NamesSc[3] = {"hDPhiLLSc","hDPhiILSc","hDPhiHLSc"};
	...
	//Reading files
	for(int i = 0; i < 3; i++){
		if( mechanism == 0 ) hDPhi[i] = Read_Hist(filename, DPhi_Names[i]);
		if( mechanism == 1 ) hDPhi[i] = Read_Hist(filename, DPhi_NamesPr[i]);
		if( mechanism == 2 ) hDPhi[i] = Read_Hist(filename, DPhi_NamesSc[i]);
	}
...
TGraphErrors *Yield_Intermediate(TString filename,Double_t start[2], Double_t finish[2],Double_t low_limit, Double_t high_limit, int mechanism){
	//This function takes the filename the start and finish for the background calculatiion and the limits for the yield calculation
	//and returns a graph where the per trigger yield is displayed when associating with low momentum particles.
	//Dictionaries used
	...
	const char* DPhi_NamesPr[2]= {"hDPhiIIPr","hDPhiHIPr"};
	const char* DPhi_NamesSc[2] = {"hDPhiIISc","hDPhiHISc"};
...
TGraphErrors *Yield_High(TString filename,Double_t start, Double_t finish,Double_t low_limit, Double_t high_limit, int mechanism){
	...
	//Reading files
	TH1D *hDPhi;
	if( mechanism == 0 ) hDPhi = Read_Hist(filename, "hDPhiHH");
	if( mechanism == 1 ) hDPhi = Read_Hist(filename, "hDPhiHHPr");
	if( mechanism == 2 ) hDPhi = Read_Hist(filename, "hDPhiHHSc");
	TH1D *hTrigPt = Read_Hist(filename,"hTrPt");
...
//For width plotting
TGraphErrors *Width_Low(TString filename,Double_t start[3], Double_t finish[3],Double_t low_limit, Double_t high_limit, int mechanism){
	const char* DPhi_Names[3] = {"hDPhiLL","hDPhiIL","hDPhiHL"};
	const char* DPhi_NamesPr[3]= {"hDPhiLLPr","hDPhiILPr","hDPhiHLPr"};
	const char* DPhi_NamesSc[3] = {"hDPhiLLSc","hDPhiILSc","hDPhiHLSc"};
...
TGraphErrors *Width_Intermediate(TString filename,Double_t start[2], Double_t finish[2],Double_t low_limit, Double_t high_limit, int mechanism){
...
TGraphErrors *Width_High(TString filename,Double_t start, Double_t finish,Double_t low_limit, Double_t high_limit, int mechanism){
	//This functions takes the filename the start and finish for the background calculatiion and the limits for the yield calculation
	//and returns a graph where the per trigger yield is displayed when associating with intermediate momentum particles.
...
void status_analysis_v2(){
	...
	*/
	...
	//plot_all("complete_root/LplusDplus_complete.root");
	//background_plot("complete_root/DplusLplus_complete.root",start,finish);
	//fit_and_plot_all("complete_root/LplusDplus_complete.root",start,finish,PI/2,3*PI/2);
	
	//Yield plots
	TGraphErrors *gr = Yield_Low("complete_root/DplusDplus_complete.root",start_l,finish_l,PI/2,3*PI/2,0);
	TGraphErrors *gr_i = Yield_Intermediate("complete_root/DplusDplus_complete.root",start_i,finish_i,PI/2,3*PI/2,0);
	TGraphErrors *gr_h = Yield_High("complete_root/DplusDplus_complete.root",start_h,finish_h,PI/2,3*PI/2,0);
	//String fragmented plots
	TGraphErrors *grPr = Yield_Low("complete_root/LplusDplus_complete.root",start_l,finish_l,PI/2,3*PI/2,0);
	TGraphErrors *gr_iPr = Yield_Intermediate("complete_root/LplusDplus_complete.root",start_i,finish_i,PI/2,3*PI/2,0);
	TGraphErrors *gr_hPr = Yield_High("complete_root/LplusDplus_complete.root",start_h,finish_h,PI/2,3*PI/2,0);
	//Decay
	TGraphErrors *grSc = Yield_Low("complete_root/DplusLplus_complete.root",start_l,finish_l,PI/2,3*PI/2,0);
	TGraphErrors *gr_iSc = Yield_Intermediate("complete_root/DplusLplus_complete.root",start_i,finish_i,PI/2,3*PI/2,0);
	TGraphErrors *gr_hSc = Yield_High("complete_root/DplusLplus_complete.root",start_h,finish_h,PI/2,3*PI/2,0);
	
	//Width plots
	TGraphErrors *grw = Width_Low("complete_root/LplusLplus_complete.root",start_l,finish_l,PI/2,3*PI/2,0);
	TGraphErrors *gr_iw = Width_Intermediate("complete_root/LplusLplus_complete.root",start_i,finish_i,PI/2,3*PI/2,0);
	TGraphErrors *gr_hw = Width_High("complete_root/LplusLplus_complete.root",start_h,finish_h,PI/2,3*PI/2,0);
	//String fragmented plots
	TGraphErrors *grPrw = Width_Low("complete_root/LplusDplus_complete.root",start_l,finish_l,PI/2,3*PI/2,0);
	TGraphErrors *gr_iPrw = Width_Intermediate("complete_root/LplusDplus_complete.root",start_i,finish_i,PI/2,3*PI/2,0);
	TGraphErrors *gr_hPrw = Width_High("complete_root/LplusDplus_complete.root",start_h,finish_h,PI/2,3*PI/2,0);
	//Decay
	TGraphErrors *grScw = Width_Low("complete_root/DplusLplus_complete.root",start_l,finish_l,PI/2,3*PI/2,0);
	TGraphErrors *gr_iScw = Width_Intermediate("complete_root/DplusLplus_complete.root",start_i,finish_i,PI/2,3*PI/2,0);
	TGraphErrors *gr_hScw = Width_High("complete_root/DplusLplus_complete.root",start_h,finish_h,PI/2,3*PI/2,0);
	
	//Plotting
	TGaxis *low = new TGaxis(8.,0.0045,16.1,0.0045,0.15,16.1,10,"-L");
	low->SetLabelColor(kRed+2);
 	low->SetLineColor(kRed+2);
  	...
  	TGaxis *inter = new TGaxis(3.,0.0045,8.,0.0045,0.15,8.,10,"-L");
	inter->SetLabelColor(kRed+2);
 	inter->SetLineColor(kRed+2);
  	...
  	TGaxis *high = new TGaxis(0.15,0.0045,3.,0.0045,0.15,3.1,10,"-L");
	high->SetLabelColor(kRed+2);
 	high->SetLineColor(kRed+2);
  	...
  	TLine *line_one = new TLine(3.,0.,3.,0.0045);
  	line_one->SetLineStyle(2);
  	line_one->SetLineWidth(1);
  	
  	TLine *line_two = new TLine(8.,0.,8.,0.0045);
  	line_two->SetLineStyle(2);
  	line_two->SetLineWidth(1);
  	
  	grPr->SetMarkerColor(2);
  	gr_iPr->SetMarkerColor(2);
  	gr_hPr->SetMarkerColor(2);
  	
  	grSc->SetMarkerColor(4);
  	gr_iSc->SetMarkerColor(4);
  	gr_hSc->SetMarkerColor(4);
  	
  	TLegend *leg = new TLegend();
  	leg->SetHeader("Correlation Type","c");
  	leg->AddEntry(gr,"#Lambda_{c}^{+}#Lambda_{c}^{+}","p");
  	leg->AddEntry(grPr,"#Lambda_{c}^{+}D^{+}","p");
  	leg->AddEntry(grSc,"D^{+}#Lambda_{c}^{+}","p");
  	leg->SetBorderSize(0);
  	
  	TCanvas *c1 = new TCanvas();
	gr->GetYaxis()->SetRangeUser(-0.00001,0.0045);
	gr->GetXaxis()->SetLimits(0.15,16.1);
	gr->Draw("A P");
	high->Draw();
	inter->Draw();
	...
	high_w->SetLabelColor(kRed+2);
 	high_w->SetLineColor(kRed+2);
  	...
  	TLine *line_one_w = new TLine(3.,0.,3.,1.5);
  	line_one_w->SetLineStyle(2);
  	line_one_w->SetLineWidth(1);
  	
  	TLine *line_two_w = new TLine(8.,0.,8.,1.5);
  	line_two_w->SetLineStyle(2);
  	line_two_w->SetLineWidth(1);
  	
  	grPrw->SetMarkerColor(2);
  	gr_iPrw->SetMarkerColor(2);
  	gr_hPrw->SetMarkerColor(2);
	
	grScw->SetMarkerColor(4);
  	gr_iScw->SetMarkerColor(4);
  	gr_hScw->SetMarkerColor(4);
	
	TCanvas *c2 = new TCanvas();
	grw->GetYaxis()->SetRangeUser(0.,1.5);
