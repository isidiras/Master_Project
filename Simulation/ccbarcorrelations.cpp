...
int main(int argc, char** argv){
	...
	Double_t pT, eta, phi, pTTriger, pTAssociate, charge, DeltaPhiDD;
	...
	for(int iEvent = 0; iEvent<nEvents; iEvent++){
			...
			//Creating correlation plots
				if(id==411){//D meson triger
					...
					for(int jPart =0; jPart<nPart; jPart++){
						...
						if( associate_id == 411){//DD correlation
							pTTriger = pT;
		...
		if(vID.empty() || vPt.empty() || vEta.empty() || vPhi.empty() || vCharge.empty()) continue; 
		tree->Fill();
	}//End of event loop
